...
<div class="container">
    <section class="col-12" style="margin-top: 15px">
        <!-- /.row -->
        <h3 class="text-center" style="margin-top: 30px">CHI TIẾT PHIẾU XUẤT HÀNG</h3>

        <div class="row invoice-info">
            <!-- /.col -->
            <!-- /.col -->
            <div class="col-sm-6 invoice-col text-center">
                <h4 style="text-transform: uppercase">Thông tin xuất hàng</h4>
                <address>
                    <b>Mã bán hàng: {{ isset($warehouse) ? $warehouse->warehouse_no : '' }}</b><br>
                    <span>Khách hàng :</span> {{ isset($warehouse) ? $warehouse->name : '' }}<br>
                    <span>Người xuất : {{ isset($warehouse->user) ? $warehouse->user->name : '' }}</span><br>
                    <span>Ghi chú : {{ isset($warehouse->note) ? $warehouse->note : '' }}</span>
                </address>
            </div>
            <div class="col-sm-6 invoice-col text-center">
                <h4 style="text-transform: uppercase">Thông tin khách hàng</h4>
                <address>
                    <b>Mã khách hàng : {{ isset($warehouse->selling) ? $warehouse->selling->customer_no : '' }}</b><br>
                    <b>Tên khách hàng: {{ isset($warehouse->selling) ? $warehouse->selling->name : '' }}</b><br>
                    <span>Email :</span> {{ isset($warehouse->selling) ? $warehouse->selling->email : '' }}<br>
                    <span>Phone : {{ isset($warehouse->selling) ? $warehouse->selling->phone : '' }}</span><br>
                    <span>Địa chỉ : {{ isset($warehouse->selling) ? $warehouse->selling->address : '' }}</span>
                </address>
            </div>
