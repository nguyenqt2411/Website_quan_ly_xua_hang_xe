                    ...
                    <div class="card-body">
                        <div class="tab-content">
                            <div class="tab-pane active" id="settings">
                                ...
                                @endif
                                <div class="form-group row">
                                    <label for="inputName2" class="col-sm-2 col-form-label">Số điện thoại  <sup class="title-sup">(*)</sup></label>
                                    <div class="col-sm-10">
                                ...
                                @if ($admin->hasRole(ADMINISTRATOR))
                                    <div class="form-group row" >
                                        <label for="inputName2" class="col-sm-2 col-form-label">Cửa hàng  <sup class="title-sup">(*)</sup></label>
                                        <div class="col-sm-10">
                                            <select name="shop_id" class="form-control">
                                                <option value="">Chọn cửa hàng</option>
                                                @if($shops)
                                                    @foreach($shops as $shop)
                                                        <option  {{old('shop_id', isset($user->shop_id) ? $user->shop_id : '') == $shop->id ? 'selected=selected' : '' }}  value="{{$shop->id}}">{{$shop->name}}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <div class="icheck-primary d-inline">
                                            <input type="radio" id="radioPrimary1" name="status" value="1" @if (isset($user)) {{ isset($user) && $user->status == 1 ? 'checked' : '' }} @else checked @endif>
                                            <label for="radioPrimary1">
                                                Hoạt động
                                            </label>
                                        </div>
                                        <div class="icheck-primary d-inline" style="margin-left: 30px;">
                                            <input type="radio" id="radioPrimary2" name="status" value="2" @if (isset($user)) {{ isset($user) && $user->status == 2 ? 'checked' : '' }} @endif>
